...
namespace heart_priv
{
	struct HeartDebugStringify
	{
		...
		HeartDebugStringify(char* tgt, size_t tgtLen, const char* names = nullptr) :
			target_(tgt), target_capacity_(tgtLen), macroified_names_(names)
		{
...
#define HEART_TO_DEBUG_STR(tgt, tgtLen, ...)                                                                           \
	heart_priv::HeartDebugStringify(tgt, tgtLen, HEART_STRINGIFY_NAMES(__VA_ARGS__))(__VA_ARGS__)

#define HEART_ERROR(title, msg, ...)                                                                                   \
	([&](...) {                                                                                                        \
		static bool s_ignored = false;                                                                                 \
		char details[2048];                                                                                            \
		return DisplayAssertError(title, msg, HEART_TO_DEBUG_STR(details, 2048, __VA_ARGS__), __FILE__, __LINE__,      \
								  &s_ignored);                                                                         \
	}() && (__debugbreak(), false))
...
#define HEART_ASSERT(expr, ...)                                                                                        \
	(void)((!!(expr)) || HEART_ERROR("HEART_ASSERT FAILED!", "HEART_ASSERT: " #expr, __VA_ARGS__))
